...
public static class DrawPrizeCommand
{
    ...
    public static async Task Execute(ServerCommandArgs args)
    {
        ...
        var requiredBalance = Math.Max(config.DrawCost, LotteryRules.MinimumBalance);
...
public static class DrawCurrencyCommand
{
    ...
    public static async Task Execute(ServerCommandArgs args)
    {
        ...
        if (before < LotteryRules.MinimumBalance)
        {
            await args.ReplyAsync($"余额不足，抽货币至少需要 {Green($"{LotteryRules.MinimumBalance} {args.Context.Configuration.Miscellaneous.CurrencyName}")}。");
            return;
        }
        ...
        for (var index = 0; index < times; index++)
        {
            ...
            if (balance < LotteryRules.MinimumBalance)
            {
                stoppedForInsufficientBalance = true;
                break;
            }
        ...
        var stoppedMessage = stoppedForInsufficientBalance
            ? $"；余额不足 {Green($"{LotteryRules.MinimumBalance} {args.Context.Configuration.Miscellaneous.CurrencyName}")}，已提前停止"
            : string.Empty;
...
internal static class LotteryRules
{
